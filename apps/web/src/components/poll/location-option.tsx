...
const LocationVoteSummary: React.FunctionComponent<{ locationId: string }> = ({
    ...
    const participantsWhoVotedYes = getParticipants(locationId, "yes");
    const participantsWhoVotedIfNeedBe = getParticipants(locationId, "ifNeedBe");
    const participantsWhoVotedNo = getParticipants(locationId, "no");
    const noVotes =
            ...
            {noVotes ? (
                ...
                <div className="grid grid-cols-2 gap-2">
                    <div className="col-span-1 space-y-2.5">
                        {participantsWhoVotedYes.map(({ name, id }) => (
                        ...
                        {participantsWhoVotedIfNeedBe.map(({ name, id }) => (
                            <div key={id} className="flex">
                                <div className="relative mr-2.5 flex size-5 items-center justify-center">
                                    <OptimizedAvatarImage size="xs" name={name} />
                                    <VoteIcon
                    ...
                    {participantsWhoVotedNo.length > 0 && (
                        <div className="col-span-1 space-y-2.5">
                            {participantsWhoVotedNo.map(({ name, id }) => (
                                <div key={id} className="flex">
                                    <div className="relative mr-2.5 flex size-5 items-center justify-center">
                                        <OptimizedAvatarImage size="xs" name={name} />
                                        <VoteIcon
...
const LocationOption: React.FunctionComponent<LocationOptionProps> = ({
    locationId,
    address,
    placeId,
    lat,
    ...
    return (
        ...
        >
            <div className="flex h-7 items-center justify-between gap-x-4">
                ...
                <div className="flex items-center gap-x-4">
                    {role === "participant" && poll.hideParticipants ? (
                        <ConnectedScoreSummary optionId={locationId} />
                    ) : (
                        <Button
                            size="sm"
                            variant="ghost"
            ...
            {isExpanded ? <LocationVoteSummary locationId={locationId} /> : null}
            {/* TODO: Add map preview when expanded */}
        </div>
    );
};
